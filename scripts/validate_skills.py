...
# dependencies = ["pyyaml"]
# ///
"""Validate the SKILL.md contract and the plugin/marketplace manifests.

Source of truth is `plugins/<domain>/skills/<name>/SKILL.md`. A folder under a
plugin's `skills/` is a skill iff it contains `SKILL.md`, whose YAML frontmatter
must hold a non-empty `name` (matching the folder) and `description`. On top of
that, this validator checks that:
...
def main() -> int:
    ...
    print(f"validated {len(skill_files)} skills and plugin manifests")
    return 0
